...
func benchmarkParseDevice(b *testing.B, ua string) {
        // if agent.Device.Regstring != expectedName {
        //      b.FailNow()
        // }
...
// device first in the list
func BenchmarkParse_FindHbbTVDevice(b *testing.B) {
        benchmarkParseDevice(b, "/^HbbTV/si")
}

// device last in list
func BenchmarkParse_FindMozillaAndroidTabletDevice(b *testing.B) {
        benchmarkParseDevice(b, "/^Mozilla.*Android.*Tablet.*AppleWebKit/si")
}
